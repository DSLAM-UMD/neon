...
/// This wrapper lets us synchronously wait for nested future's completion
...
impl<S, T: Future> Future for SyncFuture<S, T> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> task::Poll<Self::Output> {
        self.project().nested.poll(cx)
    }
}

/// Proves that we can call [`SyncFuture::wait`].
/// If implementer is also a [`Future`], it should always
/// return [`task::Poll::Ready`] from [`Future::poll`].
pub trait SyncProof {}

impl<T: SyncProof> SyncProof for &T {}
impl<T: SyncProof> SyncProof for &mut T {}

impl<P: SyncProof, T: Future> SyncFuture<P, T> {
...
/// This lets us call [`SyncFuture::wait`].
impl<T: io::Read + Unpin> SyncProof for AsyncishRead<T> {}
